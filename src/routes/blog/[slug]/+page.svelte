...
{#if showPyodidePrompt}
  <div class="fixed right-4 bottom-4 z-50">
    <Alert.Root class="bg-yellow-400 text-black">
      ...
      >
      <Alert.Description class="text-sm text-black opacity-60">
        This post contains interactive Python code blocks
      </Alert.Description>
    </Alert.Root>
  </div>
{/if}
...
<div class="fixed top-18 left-4 z-40 2xl:hidden">
...
{#if showMobileToc}
  ...
  <div
    class="bg-background fixed top-0 left-0 z-40 h-full w-80 transform overflow-y-auto border-r p-4 transition-transform duration-300 ease-in-out 2xl:hidden"
...
<article>
  <div class="mb-8 flex items-center justify-between">
    ...
    <a
      ...
      class="inline-flex items-center opacity-50 transition-opacity hover:opacity-100"
    >
  ...
  <div class="relative">
    <aside
      class="4xl:left-16 fixed top-1/2 left-2 hidden max-h-[70vh] w-64 -translate-y-1/2 overflow-y-auto 2xl:left-12 2xl:block"
    >
